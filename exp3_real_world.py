...
start_goal_ori = (R.from_quat(np.array([0.641, 0.755, -0.077, 0.109])) * 
                  R.from_euler("xyz", [0, 0, 90], degrees=True)).as_quat()
ee_min_pos_world = np.array([-0.6, -0.6, -0.16])
ee_max_pos_world = np.array([0.725, -0.2, 0.35])
num_objects = 1
object_radii = np.array([4.0] * num_objects)[:,
                                             np.newaxis]  # defined on net scale
goal_rot_radius = np.array([1.5])
goal_pos_world = np.array([-0.47447, -0.5, 0.1])
goal_ori_quat = start_goal_ori
start_pos_world = np.array([0.4, -0.5, 0.1])
start_ori_quat = start_goal_ori
scanner_pos_world = np.array([0.08, -0.89, 0.575])
...
if __name__ == "__main__":
    ...
    # saved_weights = torch.load("exp3_saved_weights.pth")
    ...
    for exp_iter in range(3):
        ...
        while not rospy.is_shutdown() and pose_error > pose_error_tol and del_pose_running_avg.avg > del_pose_tol:
            ...
            if args.view_ros:
                all_objects = [Object(pos=pose[0:POS_DIM], ori=scanner_ori_quat,
                                      radius=radius) for pose, radius in
                               zip(object_poses_net, object_radii)]
                all_objects += [
                    Object(
            ...
            if it % 2 == 0:
                print("target - cur pos: ", local_target_pos_world - cur_pos_world)
