...
def train(model: PolicyNetwork, train_args: argparse.Namespace, saved_root: str,
    ...
    train_rot = train_args.train_rot
...
def model_rollout(goal_tensors, current_tensors, start_tensors, goal_rot_inputs,
    ...
    :param goal_tensors: goal poses
    :param current_tensors: current poses
    :param start_tensors: start poses
    :param goal_rot_inputs: goal rotation inputs
    :param object_inputs: object poses
    :param obj_idx_tensors: object indices/types
    :param intervention_traj: human intervention/expert trajectory
    :param model: model/network to rollout
    :param agent_radii: agent radii
...
def adaptation_loss(model: PolicyNetwork, batch_data: List[Tuple], pos_dim, dstep,
    ...
    :param pos_dim: dimensionality of position
    ...
    for sample in batch_data:
        ...
        all_goal_rot_rs.append(goal_rot_r[np.newaxis])
        all_objs.append(objs[np.newaxis])  # 1 x 1(n_objs) x pose_dim
        all_objs_r.append(objs_r[np.newaxis])  # 1 x 1(n_objs) x 1
...
def perform_adaptation(policy: Policy, batch_data: List[Tuple],
    ...
    """
    adaptable_param_idxs = []
    adaptable_params = []
    for p_idx, p in policy.adaptable_parameters:
        adaptable_param_idxs.append(p_idx)
        adaptable_params.append(p)
        if verbose:
            print("Original p: ", p)

    optimizer = torch.optim.Adam(adaptable_params, lr=1e-1)
    losses = []
    pred_traj = None
...
def parse_arguments():
    ...
    parser.add_argument('--train_rot', action='store_true', help="whether to train rotation network")
