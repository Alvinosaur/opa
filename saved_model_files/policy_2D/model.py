...
def quat2mat(Q: torch.Tensor):
    ...
    :param Q: quaternion (batch_size, 4)
    :return: rotation matrix (batch_size, 3, 3)
    """
    q1 = Q[:, 0:1]
    q2 = Q[:, 1:2]
    q3 = Q[:, 2:3]
...
class Attention(nn.Module):
    ...
    def forward(self, edge_feats: torch.Tensor, dists: torch.Tensor):
        ...
        :param edge_feats: (batch_size, num_objects, input_size)
        :param dists: (batch_size, num_objects, 1)
        :return:
...
class RelationNetwork(nn.Module):
    ...
    def forward(self, edges: torch.Tensor, dists: torch.Tensor):
        ...
        :param edges: (batch_size, num_objects, input_size)
        :param dists: (batch_size, num_objects, 1)
        :return:
            edges: processed edge features (batch_size, num_objects, output_size)
            alpha: attention weights that sum to 1 across objects
        """
...
class PolicyNetwork(nn.Module):
    ...
    def apply_rot_offset_2D(self, input_poses, rot_offset) -> torch.Tensor:
        ...
        :param input_poses: [position, orientation(cos, sin)] (batch_size, n_objects, 4)
        :param rot_offset: learned rotation offset (batch_size, n_objects, 1)
        :return: new_orientations: [cos, sin] (batch_size, n_objects, 2)
        """
    ...
    def apply_rot_offset_3D(self, input_poses, rot_offset) -> torch.Tensor:
        ...
        :param input_poses: [position, orientation(Rx, Ry)] (batch_size, n_objects, 3 + 6)
        :param rot_offset: learned rotation offset (batch_size, n_objects, 4)
        :return: new_orientations: [Rx, Ry] (batch_size, n_objects, 6)
        ...
        orig_mats = torch.stack([Rx, Ry, Rz], dim=-1)

        # Normalize learned rotation offset quaternions
        # and convert to rotation matrices
        B, n_objects, q_shape = rot_offset.shape
        q = (rot_offset / rot_offset.norm(dim=-1, keepdim=True)).view(B * n_objects, 4)
        rot_mats = quat2mat(q)

        # Apply learned rotation offset to input object orientations
        new_mat = torch.bmm(orig_mats, rot_mats)

        # Convert new orientation matrices back to Rx, Ry
        new_Rx = new_mat[:, :, 0].view(B, n_objects, 3)
        new_Ry = new_mat[:, :, 1].view(B, n_objects, 3)
    ...
    def calc_rot_vec(self, start: torch.Tensor, goal_rot: torch.Tensor,
        ...
        :param start: start pose (batch_size, 1, pos_dim + rot_dim + 1)
        :param goal_rot: goal pose (batch_size, 1, pos_dim + rot_dim + 1)
        :param objects: actual object poses (batch_size, n_objects, pos_dim + rot_dim + 1)
        :param object_indices: object indices NOT including start/goal (batch_size, n_objects)
        :param pos_dist_ratios: size-relative distance to each object (batch_size, n_objects + 2)
        :param is_training: whether to use training or testing object features
        :return: rot_vec: (batch_size, rot_dim)
        """
        ...
        _, rot_alphas = self.rot_relation_network(
    ...
    def calc_pos_vec(self, current: torch.Tensor, goal: torch.Tensor,
        ...
        :param current: current agent/robot pose (batch_size, 1, pos_dim + rot_dim + 1)
        :param goal: goal pose (batch_size, 1, pos_dim + rot_dim + 1)
        :param objects: actual object poses (batch_size, n_objects, pos_dim + rot_dim + 1)
        :param object_indices: object indices NOT including start/goal (batch_size, n_objects)
        :param pos_dist_ratios: position distance ratios (batch_size, n_objects + 1)
        :param is_training: whether to use training or testing object features
        :return:
        """
    ...
    def forward(self, current, start, goal, goal_rot, objects, object_indices, is_training,
        ...
        :param start: start pose (batch_size, 1, pos_dim + rot_dim + 1)
        :param goal: goal pose (batch_size, 1, pos_dim + rot_dim + 1)
        :param goal_rot: goal pose for rotation network (same pose, different radius) (batch_size, 1, rot_dim + 1)
        :param objects: actual object poses (batch_size, n_objects, pos_dim + rot_dim + 1)
        :param object_indices: object indices NOT including start/goal (batch_size, n_objects)
...
class Policy(object):
    def __init__(self, policy_network):
        ...
        pos_repel_feat = self.policy_network.pos_pref_feat_train[Params.REPEL_IDX].data
        pos_attract_feat = self.policy_network.pos_pref_feat_train[Params.ATTRACT_IDX].data
        rot_ignore_feat = self.policy_network.rot_pref_feat_train[Params.IGNORE_ROT_IDX].data
        rot_care_feat = self.policy_network.rot_pref_feat_train[Params.CARE_ROT_IDX].data

        a = 0.4  # Tune this after training
        self.pos_ignore_feat = (a * pos_repel_feat + (1 - a) * pos_attract_feat)

        b = 0.75  # Tune this training
    ...
    def __call__(self, is_training=True, *args, **kwargs):
    ...
    def init_new_objs(self, pos_obj_types: List[Union[int, None]], rot_obj_types: List[Union[int, None]],
        ...
        # also append start and/or goal features
        ...
        # Make all object features adaptable, but not start/goal
        for p in self.adaptable_parameters:
    ...
    def clip_adaptable_parameters(self):
        ...
        for p in self.obj_pos_feats:
            p.data.clamp_(self.pos_pref_feat_bounds[0], self.pos_pref_feat_bounds[1])
        for p in self.obj_rot_feats:
            p.data.clamp_(self.rot_pref_feat_bounds[0], self.rot_pref_feat_bounds[1])
